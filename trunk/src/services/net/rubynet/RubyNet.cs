...
";
    ...
    static void Main() {
      ...
      RubyShell shell = factory.CreateShell(command_line);
      shell.Run();

      // start a service if we have enough information to do it or explain to
      // the caller how to use this application.

      // if the user wants to show the interactive  shell we need to start
      // the service after the shell, so the messages generated by the
      // service initialization process could be displayed on the shell
      // console window.
    }
    ...
    static void OnUnhandledException(object sender,
      ...
      RubyLogger.ForCurrentProcess.Fatal(
        "[Main   Nohros.Ruby.Service.Net.RubyNet]", (Exception)e.ExceptionObject);
    }
  }
}