...
";
        ...
        static void Main() {
            ...
            // for now we will log the messages using the pre defined file logger of the nohros library.
            //
            // TODO: create a factory to instantiate a custom logger using a configuration file.
            logger = new Log4NetLogger(FileLogger.ForCurrentProcess.Logger);
            ...
            if (command_line.HasSwitch(kAssemblyNameSwitch) && command_line.HasSwitch(kTypeNameSwitch)) {
                if (with_shell) {
                    // if the user wants the shell we need to start the service after the shell, so
                    // the messages generated by the service initialization could be displayed
                    // to the user into the shell console window.
                    rn_shell.Run(command_line.CommandLineString);
                } else {
                    rn_shell.RunAndExit(command_line);
                }
            } else if (with_shell) {
                rn_shell.Run(string.Empty);
            } else {
                logger.Info(kUsage);
            }
            return;
